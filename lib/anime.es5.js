...
var anime = (function() {
  ...
  var parseTime = function parseTime2(time, ins) {
    if (!is.str(time))
      return time;
    var t = time;
    var isPercentage = t.includes("%");
    if (isPercentage) {
      var value = validateValue(t);
  ...
  function timeline() {
    ...
    tl2.add = function(input, timelineOffset) {
      ...
      var setInsParams = function setInsParams2(ins) {
        ...
        ins.timelineOffset = is.und(timelineOffset) ? tl2.duration : getRelativeValue(parseTime(timelineOffset, tl2), tl2.duration);
      };
      switch (true) {
